...
import { getPerspectives, parsePerspectivesInput, PERSPECTIVE_REGISTRY } from "./perspectives.js";
...
describe("parsePerspectivesInput", () => {
  ...
  it('"security,performance" → ["security", "performance"]', () => {
    expect(parsePerspectivesInput("security,performance")).toEqual(["security", "performance"]);
  });
  ...
  it('"unknown,also-unknown" → ["unknown", "also-unknown"] (no validation)', () => {
    expect(parsePerspectivesInput("unknown,also-unknown")).toEqual(["unknown", "also-unknown"]);
  });
...
describe("PERSPECTIVE_REGISTRY", () => {
  ...
  it.each(Object.values(PERSPECTIVE_REGISTRY))('systemPrompt of "$name" contains "JSON"', (p) => {
