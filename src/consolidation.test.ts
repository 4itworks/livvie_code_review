...
describe('mergeSummaries', () => {
  ...
  it('high severity → changes requested emoji', () => {
    const matrix: ReviewMatrixResult = {
      results: [makeResult({ review: { summary: '🔴 Changes requested — bug found.', findings: [] } })],
      rawFindings: [],
      failedBatches: [],
      unreviewedFiles: [],
      totalCalls: 1,
  ...
  it('medium severity → warning emoji', () => {
    const matrix: ReviewMatrixResult = {
      results: [makeResult({ review: { summary: '⚠️ Review recommended — issues found.\n\nChanges in `lib/main.dart`.', findings: [] } })],
      rawFindings: [],
      failedBatches: [],
      unreviewedFiles: [],
      totalCalls: 1,
  ...
  it('findings count in summary', () => {
    ...
    const matrix: ReviewMatrixResult = {
      results: [makeResult({ review: { summary: '✅ Looks good.\n\nChanges in `lib/main.dart`.', findings: [f1, f2] } })],
      rawFindings: [f1, f2],
      failedBatches: [],
      unreviewedFiles: [],
      totalCalls: 1,
...
describe('buildPerspectiveSummaries', () => {
  it('perspective with no results → no issues message', () => {
    ...
    const result = buildPerspectiveSummaries(matrix, [makePerspective('security', 'Security Reviewer')]);
    expect(result).toHaveLength(1);
  ...
  it('error results are excluded from counts', () => {
    const matrix: ReviewMatrixResult = {
      results: [
        makeResult({ perspectiveId: 'security', review: { summary: 'test', findings: [makeFinding({ perspective: 'security' })] } }),
...
describe('consolidateReviews', () => {
  it('full pipeline: dedup + sort + cap + summaries + stats', () => {
    const findings = [
      ...
      makeFinding({ severity: 'high', file: 'lib/utils.dart', line: 20, perspective: 'generalist' }),
    ...
    const matrix: ReviewMatrixResult = {
      results: [makeResult({ perspectiveId: 'generalist', review: { summary: '✅ Looks good.\n\nChanges in `lib/main.dart`.', findings } })],
      rawFindings: findings,
      failedBatches: [],
      unreviewedFiles: [],
      totalCalls: 1,
  ...
  it('0 findings → approved summary', () => {
    const matrix: ReviewMatrixResult = {
      results: [makeResult({ perspectiveId: 'generalist', review: { summary: '✅ Looks good.', findings: [] } })],
      rawFindings: [],
      failedBatches: [],
      unreviewedFiles: [],
      totalCalls: 1,
  ...
  it('caps at 100 findings', () => {
    ...
    const matrix: ReviewMatrixResult = {
      results: [makeResult({ perspectiveId: 'generalist', review: { summary: '🔴 Changes requested.\n\nMany issues.', findings } })],
      rawFindings: findings,
      failedBatches: [],
      unreviewedFiles: [],
      totalCalls: 1,
