...
describe('parseIgnorePatterns', () => {
  ...
  it('patterns with spaces are trimmed', () => {
    expect(parseIgnorePatterns(' *.g.dart , build/** ')).toEqual([
      '*.g.dart',
      'build/**',
    ]);
  });
...
describe('filterIgnoredFiles', () => {
  it('mix of matching and non-matching files → correct split', () => {
    ...
    expect(result.kept.map((f) => f.filename)).toEqual([
      'src/main.dart',
      'src/bar.dart',
    ]);
  ...
  it('all ignored → kept=[], ignored=[all]', () => {
    const files = [
      makeDiffFile('a.g.dart'),
      makeDiffFile('b.g.dart'),
    ];
